(ns zalo.main
  "Kotodama WASM entrypoint for the Zalo clean-room actor (L4) — Clojure port.
  ...
  facts (`zalo.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"zalo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zalo-compat" :tier tier :entities entities} 200])
